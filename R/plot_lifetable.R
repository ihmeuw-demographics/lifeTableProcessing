plot_lifetable_base <- function() {

  ggplot(mapping = aes(x = age_start, y = qx)) +
...
plot_lifetable <- function(lt, open_age_var = NULL) {
  ...
  if (!is.null(open_age_var)) {
    ...
    gg <- plot_lifetable_base() +
      geom_line(
        data = lt,
        mapping = aes(color = factor(get(open_age_var)))
      ) +
      labs(color = "Input open age")

  } else {

    gg <- plot_lifetable_base() + geom_line(data = lt)

  }

  gg + ggtitle(paste(lt[1, loc_id], lt[1, year], lt[1, sex]))
